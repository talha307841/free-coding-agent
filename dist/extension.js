...
function normalizePath(path3) {
  return path3.replace(/^a\//, "").replace(/^b\//, "").trim();
}
...
function parseUnifiedDiff(diffText) {
  ...
  while (i2 < lines.length) {
    ...
    const oldPath = normalizePath(lines[i2].slice(4));
    const plusLine = lines[i2 + 1] ?? "";
    ...
    while (i2 < lines.length && !lines[i2].startsWith("--- ")) {
      ...
      i2 += 1;
      const hunkLines = [];
      while (i2 < lines.length && !lines[i2].startsWith("@@ ") && !lines[i2].startsWith("--- ")) {
        hunkLines.push(lines[i2]);
        i2 += 1;
      }
...
var ContextBuilder = class {
  ...
  async topRelevant(query, count) {
    ...
    const q2 = this.termFreq(query);
    const scored = this.indexed.map((doc) => ({
      doc,
      score: this.cosineTfidf(q2, doc.terms)
    }));
...
var ModelRouter = class {
  ...
  routeCandidates(kind2, preferred) {
    const defaults2 = {
    ...
    const unique = [];
    for (const model of defaults2[kind2]) {
  ...
  selectModel(kind2, preferred) {
    const candidates = this.routeCandidates(kind2, preferred);
    for (const model of candidates) {
      ...
      this.output.appendLine(`[router] selected model=${model} kind=${kind2} avgLatency=${avg}ms`);
      return model;
    }
...
var NimClient = class {
  ...
  async withRetry(op, model, attempts = 3) {
    ...
    for (let i2 = 0; i2 < attempts; i2 += 1) {
      try {
        ...
        if (i2 < attempts - 1) {
          await new Promise((resolve) => setTimeout(resolve, delays[i2]));
        }
      }
    }
    throw lastError;
  ...
  selectRoutedModel(kind2, preferred) {
    return this.router.selectModel(kind2, preferred);
  }
...
var ChatPanelProvider = class {
  ...
  async resolveWebviewView(view) {
    ...
    view.webview.onDidReceiveMessage(async (message) => {
      try {
        switch (message.type) {
          case "send":
            await this.onSend(message.text, message.model);
            break;
          case "stop":
          ...
          case "clear":
            ...
            break;
          case "enhancePrompt": {
            const enhanced = await this.enhancePrompt(message.text);
            this.post({ type: "enhancedPrompt", text: enhanced });
            break;
          }
          case "listFiles": {
            const query = String(message.query ?? "").toLowerCase();
            const files = (await vscode12.workspace.findFiles("**/*", "**/{.git,node_modules,dist}/**", 200)).map((uri) => vscode12.workspace.asRelativePath(uri)).filter((f2) => f2.toLowerCase().includes(query)).slice(0, 20);
  ...
  async onSend(userMessage, selectedModel) {
    if (!userMessage.trim()) {
      return;
    }
    const active = vscode12.window.activeTextEditor;
    const activeContext = active ? `ACTIVE_FILE: ${vscode12.workspace.asRelativePath(active.document.uri)}
...
${file.content}`).join("\n\n");
    ...
    const fileMentions = [...userMessage.matchAll(/@file\s+([^\s]+)/g)].map((m2) => m2[1]);
...
${userMessage}`].filter(Boolean).join("\n\n")
  ...
  async renderHtml(webview) {
    const htmlPath = path2.join(this.context.extensionPath, "src", "webview", "chatPanel.html");
    const raw = await fs2.readFile(htmlPath, "utf8");
    const token = nonce();
    const csp = [
      "default-src 'none'",
    ...
    return raw.replace(/{{NONCE}}/g, token).replace("{{CSP}}", csp);
  }
  post(payload) {
...
var NimDiagnosticFixCodeActionProvider = class {
  ...
  provideCodeActions(_document, _range, context) {
    const errors = context.diagnostics.filter((d2) => d2.severity === vscode13.DiagnosticSeverity.Error);
    if (errors.length === 0) {
      return [];
    }
...
async function runDiagnosticFix(nim, uri, diagnostics, output) {
  ...
  const errorDiagnostics = diagnostics.filter((d2) => d2.severity === vscode13.DiagnosticSeverity.Error);
  ...
  while (attempt < 2) {
    ...
    const message = errorDiagnostics.map((d2) => `line ${d2.range.start.line + 1}: ${d2.message}`).join("\n");
    const content = await nim.chat({
...
${document.getText()}`
    ...
    await document.save();
    const latestDiagnostics = vscode13.languages.getDiagnostics(uri).filter((d2) => d2.severity === vscode13.DiagnosticSeverity.Error);
  ...
  context.subscriptions.push(
    vscode15.languages.onDidChangeDiagnostics(async (event) => {
      for (const uri of event.uris) {
        const hasErrors = vscode15.languages.getDiagnostics(uri).some((d2) => d2.severity === vscode15.DiagnosticSeverity.Error);
        if (hasErrors) {
